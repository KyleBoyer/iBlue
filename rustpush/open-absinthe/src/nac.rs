...
//
// This module is the native-backend half of the open-source open-absinthe NAC
// layer. It ships exactly one path:
...
pub fn backend_diagnostic() -> Result<(), String> {
    if cfg!(all(target_os = "macos", feature = "native-nac")) {
        Ok(())
    } else {
        Err("Apple AAAbsintheContext NAC requires macOS and the native-nac feature".into())
    }
}

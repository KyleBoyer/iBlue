// Portable NAC emulator derived from mackid1993/imessage-cleanup commit
// 4f9cdfdf83080439fe2021f54bce305165483329 and the pypush NAC approach.
// This file lives in the SSPL-licensed rustpush overlay. iBlue modifications:
// external allowlisted binary only, no download/bundling, no extracted XNU
// enrichment routine, and no unique hardware identifiers in logs.
...
/// Current extractors provide the fields appropriate to the source Mac.
/// Intel keys include their encrypted IOKit properties, while Apple Silicon
/// keys intentionally leave them empty. We do not ship extracted XNU routines
/// to synthesize missing properties.
...
const NAC_BINARY_ENV: &str = "IBLUE_NAC_BINARY_FILE";
const BINARY_SHA256: &str = "74c2a8fe826a478f14f6e17b3709a8b315e8c0e0e34e3fba6b9c4eee2f4516e9";
...
/// Load the exact Apple-signed binary supported by this emulator.
///
/// iBlue deliberately neither bundles nor downloads this proprietary binary.
/// The operator must provide a read-only file extracted from Apple software.
fn get_binary() -> Result<Vec<u8>, AbsintheError> {
    let path = std::env::var_os(NAC_BINARY_ENV).ok_or_else(|| {
        AbsintheError::Other(format!(
            "portable NAC requires {NAC_BINARY_ENV} to point to the supported Apple IMDAppleServices binary"
        ))
    })?;
    let data = fs::read(&path).map_err(|error| {
        AbsintheError::Other(format!("failed to read NAC binary {}: {error}", path.to_string_lossy()))
    })?;
    let hash = format!("{:x}", Sha256::digest(&data));
    if hash != BINARY_SHA256 {
        return Err(AbsintheError::Other(format!(
            "unsupported NAC binary SHA-256: expected {BINARY_SHA256}, got {hash}"
        )));
    }
    debug!("Loaded allowlisted NAC binary ({} bytes)", data.len());
    Ok(data)
}

pub fn backend_available() -> bool {
    backend_diagnostic().is_ok()
}

pub fn backend_diagnostic() -> Result<(), String> {
    get_binary().map(|_| ()).map_err(|error| error.to_string())
}
...
impl ValidationCtx {
    ...
    pub fn new(
        ...
        // Log only non-unique diagnostics. Serial, UUID, MLB, ROM, and MAC are
        // device credentials and must never be written to logs.
        info!("NAC init: product={} board={} build={}",
            hw_config.product_name, hw_config.board_id, hw_config.os_build_num);
        info!("NAC init: rom={} bytes mac={} bytes",
            hw_config.rom.len(), hw_config.io_mac_address.len());

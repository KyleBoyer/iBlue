...
const routeDocumentation: Record<string, FastifySchema> = {
  ...
  "GET /api/v1/iblue/facetime/realtime": {
    ...
    description: "Authoritative machine-readable event map for authenticated Socket.IO call controls, incoming list/answer/decline/leave, explicit time-limited inbound-media subscriptions, outgoing live audio, and live audio/video injection into an answered incoming call. Raw microphone/camera frames are delivered only to opted-in authenticated sockets and are never persisted, placed in API events, or forwarded to webhooks. Subscriptions accept either an iBlue media-call ID or the native session ID of a call iBlue answered; each returns a stable subscription ID. Incoming live audio uses individually acknowledged 20 ms, 24 kHz mono float32 frames and persistent FaceTime AAC-ELD/PT104 encoding. Incoming live video accepts FaceTime-ready Annex-B H.265 passthrough, Annex-B H.264, or packed RGBA/BGRA frames. H.264 and raw inputs use one persistent realtime VideoToolbox-backed FFmpeg encoder for the stream, preserve aspect ratio in FaceTime's negotiated 1920x1080 camera canvas, fill unused borders with black, derive the hvc1 ImageDescription automatically, and emit HEVC/PT100 to the native FaceTime sender. Audio and video share one call-level epoch and 24 kHz RTP timestamp origin; frame events may include presentationTimeUs for source-clock synchronization and otherwise receive deterministic per-track timestamps starting at zero. Start events may provide the same mediaEpochUnixMs and rtpTimestampBase explicitly, or use iBlue's shared defaults. Both paths apply backpressure independently, are exclusively owned per track, and end the call after both owning sockets disconnect. OpenAPI cannot model Socket.IO events directly, so clients must feature-detect and consume the event definitions returned here.",
    tags: ["iBlue FaceTime"],
  },
  ...
  "GET /api/v1/iblue/facetime/call/:callId": {
    ...
    description: "Returns mode, target, lifecycle state, timestamps, converted-media/capture timing, native and web participant counts, native sender state, aggregate packet/byte totals, separate audio packet/byte totals, separate video source-frame/RTP-packet/byte totals, WebRTC counters, and any startup or transport failure for one call. Native calls also expose privacy-safe VC-control diagnostics: messages received, authenticated and sent; whether stream state was published and acknowledged; whether the control handshake is ready; participant, peer-UUID and media-key context counts; and a stable last-error code. Live calls expose their shared media epoch, RTP timestamp origin, and last audio/video presentation timestamps. Parsed Apple RTP feedback includes receive counters, burst loss, queue delay, Q13 one-way delay (plus milliseconds), video loss/frame-size buckets, bandwidth estimate, and ECN counters. These are transport diagnostics rather than a substitute for source presentation timestamps. No key, control payload, or media bytes are exposed.",
    tags: ["iBlue FaceTime"],
    params: {
      type: "object",
      required: ["callId"],
  ...
  "POST /api/v1/iblue/facetime/call": {
    ...
    description: "Accepts one audio or video upload, rings the target, waits briefly for the answered device's media graph to settle, plays the media once, and hangs up after native sender completion or at the safety deadline. On macOS, ordinary audio is converted with Apple's AudioToolbox pipeline to mono 24 kHz AAC-ELD as one 480-sample (20 ms) PT104 access unit per RTP packet. For diagnostics, nativeAudioPassthrough=true preserves a validated AAC-ELD CAF. Audio is sent by the selected iBlue profile over its native one-to-one IDS/QuickRelay session after the callee answers, media keys are ready, and the peer's authenticated Apple RTP template is observed. Native video uses HEVC/PT100 on the same one-to-one leg, waits for an acknowledged camera-state transaction and peer PT100 activity (with a bounded camera-off fallback), and trims camera media to the uploaded audio duration so completion and hangup stay aligned. Remote HEVC rendering, synchronized audio, and automatic hangup have been verified on an answered iPhone call. The lifecycle exposes separate audio/video totals and fails rather than reporting success when a sender cannot start. It does not involve FaceTime.app or a web guest on native-capable macOS. Only one deterministic media call may run at a time.",
    tags: ["iBlue FaceTime"],
    consumes: ["multipart/form-data"],
    body: {
      type: "object",
      ...
      properties: {
        ...
        nativeVideoPassthrough: { type: "boolean", default: false, description: "Video-only diagnostic mode that preserves an uploaded hvc1/HEVC video track instead of re-encoding it. iBlue rebuilds Apple's canonical hvc1 ImageDescription from the source VPS/SPS/PPS, inserts access-unit delimiters, and sends PT100 over the native one-to-one QuickRelay leg. The upload must also contain an audio track." },
      },
      anyOf: [{ required: ["address"] }, { required: ["targets"] }],
    },
  },

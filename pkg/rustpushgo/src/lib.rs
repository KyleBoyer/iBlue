...
fn bridge_default_provider(
    ...
    // The current ClearADI, remote-anisette, and AOSKit providers all persist
    // their device-specific state under `path`; the hardware-derived device ID
    // is already carried by the OSConfig used by rustpush.
...
#[derive(CloudKitRecord, Debug, Clone)]
#[cloudkit_record(type = "CMMRoot", rename_all = "camelCase")]
struct ICloudPhotosProbeRoot {
    asset_count: i64,
    photos_count: i64,
    videos_count: i64,
    start_date: std::time::SystemTime,
    end_date: std::time::SystemTime,
    create_date: std::time::SystemTime,
    #[cloudkit(encrypted)]
    preview_data: Option<rustpush::cloudkit_proto::Asset>,
}

impl Default for ICloudPhotosProbeRoot {
    fn default() -> Self {
        Self {
            asset_count: 0,
            photos_count: 0,
            videos_count: 0,
            start_date: std::time::UNIX_EPOCH,
            end_date: std::time::UNIX_EPOCH,
            create_date: std::time::UNIX_EPOCH,
            preview_data: None,
        }
    }
}
...
#[derive(CloudKitRecord, Debug, Clone)]
#[cloudkit_record(type = "CPLMaster", rename_all = "camelCase")]
struct ICloudPhotosProbeMaster {
    item_type: String,
    #[cloudkit(encrypted)]
    filename_enc: Vec<u8>,
    original_orientation: i64,
    record_modification_date: std::time::SystemTime,
    import_date: std::time::SystemTime,
    data_class_type: i64,
    media_meta_data_type: String,
    #[cloudkit(encrypted)]
    media_meta_data_enc: Vec<u8>,
    original_creation_date: std::time::SystemTime,
    res_original_fingerprint: String,
    res_original_file_type: String,
    res_original_file_size: i64,
    res_original_width: i64,
    res_original_height: i64,
    #[cloudkit(encrypted)]
    res_original_res: rustpush::cloudkit_proto::Asset,
    #[cloudkit(rename = "resJPEGMedFingerprint")]
    res_jpeg_med_fingerprint: String,
    #[cloudkit(rename = "resJPEGMedFileType")]
    res_jpeg_med_file_type: String,
    #[cloudkit(rename = "resJPEGMedFileSize")]
    res_jpeg_med_file_size: i64,
    #[cloudkit(rename = "resJPEGMedWidth")]
    res_jpeg_med_width: i64,
    #[cloudkit(rename = "resJPEGMedHeight")]
    res_jpeg_med_height: i64,
    #[cloudkit(encrypted, rename = "resJPEGMedRes")]
    res_jpeg_med_res: rustpush::cloudkit_proto::Asset,
    #[cloudkit(rename = "resJPEGThumbFingerprint")]
    res_jpeg_thumb_fingerprint: String,
    #[cloudkit(rename = "resJPEGThumbFileType")]
    res_jpeg_thumb_file_type: String,
    #[cloudkit(rename = "resJPEGThumbFileSize")]
    res_jpeg_thumb_file_size: i64,
    #[cloudkit(rename = "resJPEGThumbWidth")]
    res_jpeg_thumb_width: i64,
    #[cloudkit(rename = "resJPEGThumbHeight")]
    res_jpeg_thumb_height: i64,
    #[cloudkit(encrypted, rename = "resJPEGThumbRes")]
    res_jpeg_thumb_res: rustpush::cloudkit_proto::Asset,
}

impl Default for ICloudPhotosProbeMaster {
    fn default() -> Self {
        Self {
            item_type: String::new(),
            filename_enc: Vec::new(),
            original_orientation: 0,
            record_modification_date: std::time::UNIX_EPOCH,
            import_date: std::time::UNIX_EPOCH,
            data_class_type: 0,
            media_meta_data_type: String::new(),
            media_meta_data_enc: Vec::new(),
            original_creation_date: std::time::UNIX_EPOCH,
            res_original_fingerprint: String::new(),
            res_original_file_type: String::new(),
            res_original_file_size: 0,
            res_original_width: 0,
            res_original_height: 0,
            res_original_res: Default::default(),
            res_jpeg_med_fingerprint: String::new(),
            res_jpeg_med_file_type: String::new(),
            res_jpeg_med_file_size: 0,
            res_jpeg_med_width: 0,
            res_jpeg_med_height: 0,
            res_jpeg_med_res: Default::default(),
            res_jpeg_thumb_fingerprint: String::new(),
            res_jpeg_thumb_file_type: String::new(),
            res_jpeg_thumb_file_size: 0,
            res_jpeg_thumb_width: 0,
            res_jpeg_thumb_height: 0,
            res_jpeg_thumb_res: Default::default(),
        }
    }
}

#[derive(CloudKitRecord, Debug, Clone)]
#[cloudkit_record(type = "CPLAsset", rename_all = "camelCase")]
struct ICloudPhotosProbeAsset {
    asset_date: std::time::SystemTime,
    #[cloudkit(encrypted)]
    shared_sync_sharing_state_enc: Vec<u8>,
    orientation: i64,
    syndication_id: String,
    added_date: std::time::SystemTime,
    adjustment_source_type: i64,
    is_expunged: i64,
    time_zone_offset: i64,
    master_ref: rustpush::cloudkit_proto::record::Reference,
    #[cloudkit(encrypted)]
    time_zone_name_enc: Vec<u8>,
    adjustment_render_type: i64,
    is_hidden: i64,
    duration: i64,
    burst_flags: i64,
    record_modification_date: std::time::SystemTime,
    is_deleted: i64,
    most_recent_added_date: std::time::SystemTime,
    is_favorite: i64,
}

impl Default for ICloudPhotosProbeAsset {
    fn default() -> Self {
        Self {
            asset_date: std::time::UNIX_EPOCH,
            shared_sync_sharing_state_enc: Vec::new(),
            orientation: 0,
            syndication_id: String::new(),
            added_date: std::time::UNIX_EPOCH,
            adjustment_source_type: 0,
            is_expunged: 0,
            time_zone_offset: 0,
            master_ref: Default::default(),
            time_zone_name_enc: Vec::new(),
            adjustment_render_type: 0,
            is_hidden: 0,
            duration: 0,
            burst_flags: 0,
            record_modification_date: std::time::UNIX_EPOCH,
            is_deleted: 0,
            most_recent_added_date: std::time::UNIX_EPOCH,
            is_favorite: 0,
        }
    }
}

pub struct ICloudPhotosProbeMedia {
    pub path: String,
    pub filename: String,
    pub width: i64,
    pub height: i64,
...
#[derive(CloudKitRecord, Debug, Clone)]
#[cloudkit_record(type = "CMMRootPlugin", rename_all = "camelCase")]
struct ICloudPhotosProbeRootPlugin {
    expiry_date: std::time::SystemTime,
}

impl Default for ICloudPhotosProbeRootPlugin {
    fn default() -> Self {
        Self {
            expiry_date: std::time::UNIX_EPOCH,
        }
    }
}

#[derive(CloudKitRecord, Debug, Default, Clone)]
#[cloudkit_record(type = "cloudkit.share")]
struct ICloudPhotosProbeShareFields {
    #[cloudkit(rename = "cloudkit.type")]
    share_type: String,
    #[cloudkit(rename = "cloudkit.title")]
    title: String,
}

#[derive(CloudKitRecord, Debug, Default, Clone)]
#[cloudkit_record(type = "ZoneUpdatePlugin")]
struct ICloudPhotosProbeZoneUpdatePlugin {
    #[cloudkit(rename = "___zoneUpdateData")]
    zone_update_data: Vec<u8>,
}

pub struct ICloudPhotosShareProbe {
    pub share_url: String,
    pub share_routing_url: Option<String>,
    pub zone_name: String,
    pub short_guid_length: usize,
    pub has_stable_url: bool,
    pub participant_count: usize,
    pub chain_private_key_bytes: usize,
    pub has_chain_protection_info: bool,
    pub saved_media_records: usize,
    pub root_protection_bytes: usize,
    pub chain_protection_bytes: usize,
    pub invited_pcs_bytes: usize,
    pub self_added_pcs_bytes: usize,
    pub protected_full_token_bytes: usize,
    pub encrypted_public_sharing_key_bytes: usize,
    pub stable_short_token_hash_matches: bool,
    pub share_short_token_hash_matches: bool,
    pub root_protection_base64: String,
    pub self_added_pcs_base64: String,
    pub public_sharing_identity_base64: String,
    pub encrypted_public_sharing_key_base64: String,
    pub protected_full_token_base64: String,
    pub chain_private_key_base64: String,
    pub chain_protection_base64: String,
    pub invited_pcs_base64: String,
}

fn take_icloud_photos_uploaded_asset(
    uploaded: &mut HashMap<String, Vec<rustpush::cloudkit_proto::Asset>>,
    field: &str,
) -> Result<rustpush::cloudkit_proto::Asset, WrappedError> {
    uploaded
        .get_mut(field)
        .and_then(|assets| (!assets.is_empty()).then(|| assets.remove(0)))
        .ok_or_else(|| WrappedError::GenericError {
            msg: format!("Photos CloudKit omitted uploaded asset field {field}"),
        })
}

fn icloud_photos_empty_binary_plist() -> Result<Vec<u8>, WrappedError> {
    ...
    .map_err(|error| WrappedError::GenericError {
        msg: format!("failed to encode iCloud Photos metadata: {error}"),
    })?;
    Ok(bytes)
}

fn icloud_photos_probe_jpeg_rendition(bytes: &[u8], label: &str) -> Result<Vec<u8>, WrappedError> {
    if !bytes.starts_with(&[0xff, 0xd8]) {
        return Err(WrappedError::GenericError {
            msg: "the iCloud Photos media probe currently requires JPEG input".into(),
        });
    }
    let comment = format!("iBlue CMM probe {label}").into_bytes();
    let segment_len = u16::try_from(comment.len() + 2).map_err(|_| WrappedError::GenericError {
        msg: "the iCloud Photos media probe JPEG marker is too large".into(),
    })?;
    let mut rendition = Vec::with_capacity(bytes.len() + comment.len() + 4);
    rendition.extend_from_slice(&bytes[..2]);
    rendition.extend_from_slice(&[0xff, 0xfe]);
    rendition.extend_from_slice(&segment_len.to_be_bytes());
    rendition.extend_from_slice(&comment);
    rendition.extend_from_slice(&bytes[2..]);
    Ok(rendition)
}

static ICLOUD_PHOTOS_PCS_SERVICE: rustpush::pcs::PCSService<'static> =
    rustpush::pcs::PCSService {
        name: "Photos",
        view_hint: "PCS-Photos",
...
impl WrappedTokenProvider {
    /// Verify that the restored Apple account can open the private iCloud
    /// Photos CloudKit container. This is intentionally read-only: ckAppInit
    /// authenticates the container and returns the container-scoped user ID,
    /// but does not create a zone or modify Photos data.
    pub async fn probe_icloud_photos_container(&self) -> Result<bool, WrappedError> {
        static PHOTOS_CONTAINER: rustpush::cloudkit::CloudKitContainer<'static> =
            rustpush::cloudkit::CloudKitContainer {
                database_type:
                    rustpush::cloudkit_proto::request_operation::header::Database::PrivateDb,
                bundleid: "com.apple.photos.cloud",
                containerid: "com.apple.photos.cloud",
                env: rustpush::cloudkit_proto::request_operation::header::ContainerEnvironment::Production,
            };

        let (_, cloudkit) = create_keychain_clients(self).await?;
        let container = PHOTOS_CONTAINER.init(cloudkit).await?;
        Ok(!container.user_id.is_empty())
    }

    /// Create and immediately remove an empty CMM zone to verify the Photos
    /// CKShare shape. No media is uploaded and the disposable zone is deleted
    /// before this method returns, including when record creation fails.
    pub async fn probe_icloud_photos_share(
        &self,
        media: Option<ICloudPhotosProbeMedia>,
        delete_after: bool,
    ) -> Result<ICloudPhotosShareProbe, WrappedError> {
        use rustpush::cloudkit::{
            create_public_share_stable_url, create_share,
            record_identifier, CloudKitSession, CloudKitUploadRequest, FetchRecordOperation,
            SaveRecordOperation, ZoneDeleteOperation, ZoneSaveOperation,
        };
        use rustpush::cloudkit_proto::{
            identifier, record, Identifier, ShareIdentifier,
        };
        use rustpush::mmcs::prepare_put_photos;
        ...
        let container = PHOTOS_CONTAINER.init(cloudkit.clone()).await?;
        let zone = container.private_zone(format!(
            "CMM-{}",
        ...
        let zone_name = zone
            ...
            .expect("new Photos zone is missing its name");
        if let Ok(prefix) = std::env::var("IBLUE_CMM_DIAG_PREFIX") {
            let _ = std::fs::write(format!("{prefix}-zone-name.txt"), &zone_name);
            if let Some(owner_name) = zone.owner_identifier.as_ref().and_then(|value| value.name.as_ref()) {
                let _ = std::fs::write(format!("{prefix}-zone-owner.txt"), owner_name);
            }
        }
        // Fetching the zone config creates a missing zone with Jade's actual
        // Photos PCS identity. Apple rejects random PCS access keys even when
        // their protection envelope is otherwise well formed.
        let mut zone_config = container
            .get_zone_encryption_config(&zone, &keychain, &ICLOUD_PHOTOS_PCS_SERVICE)
            .await?;
        let sharer = rustpush::pcs::PCSPrivateKey::get_service_key(
            &keychain,
            &ICLOUD_PHOTOS_PCS_SERVICE,
            cloudkit.config.as_ref(),
        )
        .await?;

        let create_result: Result<ICloudPhotosShareProbe, WrappedError> = async {
            let now = std::time::SystemTime::now();
            let session = CloudKitSession::new();
            let uploaded_photo = if let Some(media) = media.as_ref() {
                let bytes = std::fs::read(&media.path).map_err(|error| WrappedError::GenericError {
                    msg: format!("failed to read iCloud Photos probe media: {error}"),
                })?;
                if bytes.is_empty() {
                    return Err(WrappedError::GenericError {
                        msg: "iCloud Photos probe media is empty".into(),
                    });
                }
                let master_id = base64::engine::general_purpose::URL_SAFE_NO_PAD
                    .encode(rand::random::<[u8; 21]>());
                let asset_id = uuid::Uuid::new_v4().to_string().to_uppercase();
                let upload_specs = [
                    ("previewData", "cmm-root", "CMMRoot"),
                    ("resOriginalRes", master_id.as_str(), "CPLMaster"),
                    ("resJPEGMedRes", master_id.as_str(), "CPLMaster"),
                    ("resJPEGThumbRes", master_id.as_str(), "CPLMaster"),
                ];
                let rendition_nonce = uuid::Uuid::new_v4().to_string().to_uppercase();
                let renditions = [
                    icloud_photos_probe_jpeg_rendition(
                        &bytes,
                        &format!("preview-{rendition_nonce}"),
                    )?,
                    icloud_photos_probe_jpeg_rendition(
                        &bytes,
                        &format!("original-{rendition_nonce}"),
                    )?,
                    icloud_photos_probe_jpeg_rendition(
                        &bytes,
                        &format!("medium-{rendition_nonce}"),
                    )?,
                    icloud_photos_probe_jpeg_rendition(
                        &bytes,
                        &format!("thumbnail-{rendition_nonce}"),
                    )?,
                ];
                // MMCS deduplicates by content signature. Make each probe
                // rendition unique so a receipt cannot refer to an older
                // upload encrypted with a different per-asset key.
                let mut uploads = Vec::with_capacity(upload_specs.len());
                for ((field, record_id, record_type), rendition) in
                    upload_specs.into_iter().zip(renditions)
                {
                    if let Ok(prefix) = std::env::var("IBLUE_CMM_DIAG_PREFIX") {
                        let _ = std::fs::write(format!("{prefix}-{field}.jpg"), &rendition);
                    }
                    let mut cursor = Cursor::new(rendition);
                    let prepared = prepare_put_photos(
                        rustpush::FileContainer::new(&mut cursor),
                    ).await?;
                    if let (Ok(prefix), Some(asset_key)) = (
                        std::env::var("IBLUE_CMM_DIAG_PREFIX"),
                        prepared.ford_key.as_ref(),
                    ) {
                        let _ = std::fs::write(format!("{prefix}-{field}-asset-key.bin"), asset_key);
                    }
                    cursor.set_position(0);
                    uploads.push(CloudKitUploadRequest {
                        file: Some(cursor),
                ...
                for upload in uploads {
                    for (field, mut assets) in container
                        .upload_asset(&session, &zone, vec![upload])
                        .await?
                    {
                        uploaded.entry(field).or_insert_with(Vec::new).append(&mut assets);
                    }
                }
                Some((
                    master_id,
                    asset_id,
                    take_icloud_photos_uploaded_asset(&mut uploaded, "previewData")?,
                    take_icloud_photos_uploaded_asset(&mut uploaded, "resOriginalRes")?,
                    take_icloud_photos_uploaded_asset(&mut uploaded, "resJPEGMedRes")?,
                    take_icloud_photos_uploaded_asset(&mut uploaded, "resJPEGThumbRes")?,
                ))
            } else {
                None
            };
            let share_identifier = ShareIdentifier {
                value: Some(Identifier {
                    name: Some("cmm-share".to_string()),
                    r#type: Some(identifier::Type::Share as i32),
                }),
                zone_identifier: Some(zone.clone()),
            };

            let mut share_info = create_share(&zone, "cmm-share", &sharer)?;
            share_info.displayed_hostname = Some("share.icloud.com".to_string());
            // CMM links use the Photos public resolver rather than a generic
            // anonymous CKShare participant.
            share_info.public_access = Some(2);
            share_info.annonymous_public_access = Some(false);
            share_info.allow_anyone_to_resolve_share = None;
            share_info.publisher_model_type = Some(1);
            share_info.participant_self_removal_behavior = None;
            share_info.requesting_access = Some(Default::default());
            share_info.blocked_access = Some(Default::default());
            share_info.pcs_invited_keys_to_remove = None;
            share_info.pcs_added_keys_to_remove = None;
            if let Some(owner) = share_info.participants.first_mut() {
                owner.user_id = Some(Identifier {
                    name: Some(container.user_id.clone()),
                    r#type: Some(identifier::Type::User as i32),
                });
                owner.public_key_version = Some(1);
            }
            let owner_protection = PCSShareProtection::from_protection_info(
                share_info
                    .participants
                    .first()
                    .and_then(|participant| participant.protection_info.as_ref())
                    .ok_or(WrappedError::GenericError {
                        msg: "Photos CMM owner participant is missing PCS protection info".into(),
                    })?,
            );
            let owner_participant_key = owner_protection
                .get_inner_keys()
                .into_iter()
                .next()
                .ok_or(WrappedError::GenericError {
                    msg: "Photos CMM owner participant has no PCS key".into(),
                })?;

            // Public shares still have a two-stage PCS envelope. invitedPCS
            // wraps the private key that unlocks selfAddedPCS; selfAddedPCS
            // carries the private key represented in the public share token.
            let invited_key = CompactECKey::new()?;
            let self_added_key = CompactECKey::new()?;
            // CloudKit web sharing uses a record-local random identity. It is
            // separate from both CKShare PCS identities: the normal share
            // flow adds invited/self-added PCS access first, then
            // CKModifyWebSharingOperation adds this public identity to the
            // root record PCS and embeds its private form in the URL token.
            let public_sharing_key = CompactECKey::new()?;
            let invited_protection = PCSShareProtection::create(
                &owner_participant_key,
                &[invited_key.clone()],
                &[self_added_key.clone()],
                PCSKey::random(),
                None,
                &[],
                None,
            ...
            share_info.invited_pcs = Some(invited_protection.to_protection_info(false)?);
            ...
            share_info.self_added_pcs = Some(self_added_protection.to_protection_info(false)?);

            // Publishing a share extends the zone PCS so the invited share
            // key can unlock it without rotating the zone's record-wrapping
            // key. CloudKit carries this zone mutation as
            // private plugin metadata on the share record and applies it in
            // the same atomic transaction as the root/share/media records.
            // Record saves are encrypted with the rolled config, but their
            // zone PCS tag remains the pre-roll value: it is the optimistic
            // locking token CloudKit compares before applying the embedded
            // zone update.
            let current_zone_protection_tag = zone_config.zone_protection_tag();
            let zone_update = ZoneSaveOperation::share_zone_keys(
                &mut zone_config,
                &sharer.key(),
                &invited_key,
            )?;

            let share_record_id = record_identifier(zone.clone(), "cmm-share");

            // CMM roots use per-record PCS protection (not the zone's unified
            // default-record key). The same record key encrypts the stable
            // URL's public sharing key and becomes the resolver's chain key.
            let root_record_id = record_identifier(zone.clone(), "cmm-root");
            let media_count = if uploaded_photo.is_some() { 1 } else { 0 };
            let (mut root, chain_tag, root_encryptor, chain_key) =
                SaveRecordOperation::new_chain_root(
                    root_record_id.clone(),
                    ICloudPhotosProbeRoot {
                    asset_count: media_count,
                    photos_count: media_count,
                    videos_count: 0,
                    start_date: now,
                    end_date: now,
                    create_date: now,
                    preview_data: uploaded_photo.as_ref().map(|photo| photo.2.clone()),
                    },
                    &zone_config,
                    &[
                        invited_key.clone(),
                        self_added_key.clone(),
                        public_sharing_key.clone(),
                    ],
                )?;
            if let (Ok(prefix), Some(photo), Some(record_key)) = (
                std::env::var("IBLUE_CMM_DIAG_PREFIX"),
                uploaded_photo.as_ref(),
                root_encryptor.keys.first(),
            ) {
                let preview = &photo.2;
                if let Some(asset_key) = preview
                    .protection_info
                    .as_ref()
                    .and_then(|info| info.protection_info.as_ref())
                {
                    let _ = std::fs::write(format!("{prefix}-asset-key.bin"), asset_key);
                }
                let _ = std::fs::write(format!("{prefix}-asset-signature.bin"), preview.signature());
                let _ = std::fs::write(
                    format!("{prefix}-asset-reference-signature.bin"),
                    preview.reference_signature(),
                );
                let _ = std::fs::write(format!("{prefix}-record-key.bin"), record_key.key_bytes());
                if let Some(wrapped_asset_key) = root
                    .0
                    .record
                    .as_ref()
                    .and_then(|record| record.record_field.iter().find(|field| {
                        field.identifier.as_ref().and_then(|identifier| identifier.name.as_deref())
                            == Some("previewData")
                    }))
                    .and_then(|field| field.value.as_ref())
                    .and_then(|value| value.asset_value.as_ref())
                    .and_then(|asset| asset.protection_info.as_ref())
                    .and_then(|info| info.protection_info.as_ref())
                {
                    let _ = std::fs::write(
                        format!("{prefix}-wrapped-asset-key.bin"),
                        wrapped_asset_key,
                    );
                }
            }
            let public_share = create_public_share_stable_url(
                &root_encryptor,
                &public_sharing_key,
                "share.icloud.com",
                true,
            )?;
            let short_token = public_share.short_token.clone();
            let short_token_length = public_share.short_token.len();
            let public_sharing_identity_base64 =
                BASE64_STANDARD.encode(&public_share.private_key_ref);
            share_info.short_token_hash = public_share.stable_url.short_token_hash.clone();
            let stable_url = public_share.stable_url;
            let root_record = root.0.record.as_mut().expect("root save is missing its record");
            root_record.plugin_fields = cloudkit_proto::CloudKitRecord::to_record(
                &ICloudPhotosProbeRootPlugin {
                    expiry_date: now + Duration::from_secs(30 * 24 * 60 * 60),
                },
            );

            let mut share = SaveRecordOperation::new(
                share_record_id.clone(),
                ICloudPhotosProbeShareFields {
                    share_type: "cmm".to_string(),
                    title: String::new(),
                },
                None,
                false,
            );
            let share_record = share.0.record.as_mut().expect("share save is missing its record");
            share_record.share_info = Some(share_info);
            share_record.plugin_fields = cloudkit_proto::CloudKitRecord::to_record(
                &ICloudPhotosProbeZoneUpdatePlugin {
                    zone_update_data: zone_update
                        .0
                        .zone
                        .as_ref()
                        .expect("Photos zone key roll is missing its zone")
                        .encode_to_vec(),
                },
            );

            let mut media_operations = Vec::new();
            if let (Some(media), Some((master_id, asset_id, _, original, medium, thumbnail))) =
                (media.as_ref(), uploaded_photo.as_ref())
            {
                let master_record_id = record_identifier(zone.clone(), master_id);
                let master = SaveRecordOperation::new_chain_child(
                    master_record_id.clone(),
                    ICloudPhotosProbeMaster {
                        item_type: "public.jpeg".into(),
                        filename_enc: media.filename.as_bytes().to_vec(),
                        original_orientation: 1,
                        record_modification_date: now,
                        import_date: now,
                        data_class_type: 1,
                        media_meta_data_type: "CGImageProperties".into(),
                        media_meta_data_enc: icloud_photos_empty_binary_plist()?,
                        original_creation_date: now,
                        res_original_fingerprint: BASE64_STANDARD.encode(original.signature()),
                        res_original_file_type: "public.jpeg".into(),
                        res_original_file_size: original.size() as i64,
                        res_original_width: media.width,
                        res_original_height: media.height,
                        res_original_res: original.clone(),
                        res_jpeg_med_fingerprint: BASE64_STANDARD.encode(medium.signature()),
                        res_jpeg_med_file_type: "public.jpeg".into(),
                        res_jpeg_med_file_size: medium.size() as i64,
                        res_jpeg_med_width: media.width,
                        res_jpeg_med_height: media.height,
                        res_jpeg_med_res: medium.clone(),
                        res_jpeg_thumb_fingerprint: BASE64_STANDARD.encode(thumbnail.signature()),
                        res_jpeg_thumb_file_type: "public.jpeg".into(),
                        res_jpeg_thumb_file_size: thumbnail.size() as i64,
                        res_jpeg_thumb_width: media.width,
                        res_jpeg_thumb_height: media.height,
                        res_jpeg_thumb_res: thumbnail.clone(),
                    },
                    &zone_config,
                    root_record_id.clone(),
                    &chain_key,
                ...
                let asset = SaveRecordOperation::new_chain_child(
                    record_identifier(zone.clone(), asset_id),
                    ICloudPhotosProbeAsset {
                        asset_date: now,
                        shared_sync_sharing_state_enc: icloud_photos_empty_binary_plist()?,
                        orientation: 1,
                        syndication_id: format!("at_0_{asset_id}"),
                        added_date: now,
                        adjustment_source_type: 0,
                        is_expunged: 0,
                        time_zone_offset: 0,
                        master_ref: record::Reference {
                            r#type: Some(record::reference::Type::Owning as i32),
                            record_identifier: Some(master_record_id),
                        },
                        time_zone_name_enc: b"UTC".to_vec(),
                        adjustment_render_type: 0,
                        is_hidden: 0,
                        duration: 0,
                        burst_flags: 0,
                ...
                media_operations.extend([master, asset]);
            }

            // CloudKit creates a public short GUID as a side effect of the
            // atomic root/share save. Saving an orphan share and attaching it
            // to an existing root later produces private routing metadata but
            // does not publish that route through records/resolve.
            let root_record = root.0.record.as_mut().expect("root save is missing its record");
            root_record.share_id = Some(share_identifier.clone());
            root_record.stable_url = Some(stable_url);
            root.0.share_id_update = Some(
                rustpush::cloudkit_proto::record_save_request::ShareIdUpdate {
                    share_id: Some(share_identifier.clone()),
                },
            );
            let mut publish_operations = vec![share, root];
            publish_operations.append(&mut media_operations);
            for operation in &mut publish_operations {
                if operation.0.zone_protection_info_tag.is_some() {
                    operation.0.zone_protection_info_tag = current_zone_protection_tag.clone();
                }
            }
            let publish_results = container
                .perform_operations(&session, &publish_operations, IsolationLevel::Zone)
                .await?;
            if publish_results.iter().any(Result::is_err) {
                return Err(WrappedError::GenericError {
                    msg: format!(
                        "Photos CMM record save failed ({})",
                        publish_results
                            .iter()
                            .enumerate()
                            .map(|(index, result)| result
                                ...
                                .unwrap_or_else(|| format!("{index}:ok")))
                            .collect::<Vec<_>>()
                            .join(", "),
                    ),
                });
            }
            let mut publish_results = publish_results.into_iter();
            let share_record = publish_results
                .next()
                .ok_or(WrappedError::GenericError {
                    msg: "Photos CloudKit omitted the CMM share save result".into(),
                })??
                .ok_or(WrappedError::GenericError {
                    msg: "Photos CloudKit did not return the saved CMM share record".into(),
                })?;
            let root_record = publish_results
                .next()
                .ok_or(WrappedError::GenericError {
                    msg: "Photos CloudKit omitted the CMM root save result".into(),
                })??
                .ok_or(WrappedError::GenericError {
                    msg: "Photos CloudKit did not return the saved CMM root record".into(),
                })?;
            let media_results = publish_results.collect::<Vec<_>>();
            if media_results.iter().any(Result::is_err) {
                return Err(WrappedError::GenericError {
                    msg: format!(
                        "Photos CMM media record save failed ({})",
                        media_results
                            .iter()
                            .enumerate()
                            .map(|(index, result)| result
            ...
            let routing_key = root_record
                .stable_url
                .as_ref()
                .and_then(|stable| stable.routing_key.clone())
                .ok_or(WrappedError::GenericError {
                    msg: "Photos CloudKit did not assign the CMM share a routing key".into(),
            })?;
            let refreshed_share = container
                .perform(
                    &session,
                    FetchRecordOperation::new(&Default::default(), share_record_id),
                )
                .await?;
            let info = refreshed_share
                .get_raw_record()
                .share_info
                .clone()
                .or(share_record.share_info)
                .unwrap_or_default();
            eprintln!(
                "CMM_DIAG root_share={} share_route={} public={:?} anonymous={:?} allow_resolve={:?} publisher={:?}",
                root_record.share_id.is_some(),
                info.short_token_routing_key.is_some(),
                info.public_access,
                info.annonymous_public_access,
                info.allow_anyone_to_resolve_share,
                info.publisher_model_type,
            );
            let stable_url = root_record.stable_url;
            let share_routing_url = info
                .short_token_routing_key
                .as_ref()
                .map(|routing_key| format!("https://share.icloud.com/photos/{routing_key}{short_token}"));
            Ok(ICloudPhotosShareProbe {
                share_url: format!("https://share.icloud.com/photos/{routing_key}{short_token}"),
                share_routing_url,
                zone_name: zone_name.clone(),
                short_guid_length: stable_url
                    .as_ref()
                    .and_then(|stable| stable.routing_key.as_ref())
                    .map(String::len)
                    .unwrap_or(0)
                    + short_token_length,
                has_stable_url: stable_url.is_some(),
                participant_count: info.participants.len(),
                chain_private_key_bytes: root_record
                    .chain_private_key
                    .as_ref()
                    .map(Vec::len)
                    .unwrap_or(0),
                has_chain_protection_info: root_record.chain_protection_info.is_some(),
                saved_media_records: media_results.len(),
                root_protection_bytes: root_record
                    .protection_info
                    .as_ref()
                    .map(|info| info.protection_info().len())
                    .unwrap_or(0),
                chain_protection_bytes: root_record
                    .chain_protection_info
                    .as_ref()
                    .map(|info| info.protection_info().len())
                    .unwrap_or(0),
                invited_pcs_bytes: info
                    .invited_pcs
                    .as_ref()
                    .map(|pcs| pcs.protection_info().len())
                    .unwrap_or(0),
                self_added_pcs_bytes: info
                    .self_added_pcs
                    .as_ref()
                    .map(|pcs| pcs.protection_info().len())
                    .unwrap_or(0),
                protected_full_token_bytes: stable_url
                    .as_ref()
                    .and_then(|stable| stable.protected_full_token.as_ref())
                    .map(Vec::len)
                    .unwrap_or(0),
                encrypted_public_sharing_key_bytes: stable_url
                    .as_ref()
                    .and_then(|stable| stable.encrypted_public_sharing_key.as_ref())
                    .map(Vec::len)
                    .unwrap_or(0),
                stable_short_token_hash_matches: stable_url
                    .as_ref()
                    .and_then(|stable| stable.short_token_hash.as_ref())
                    == Some(&openssl::sha::sha256(short_token.as_bytes()).to_vec()),
                share_short_token_hash_matches: info.short_token_hash.as_ref()
                    == Some(&openssl::sha::sha256(short_token.as_bytes()).to_vec()),
                root_protection_base64: root_record
                    .protection_info
                    .as_ref()
                    .map(|pcs| BASE64_STANDARD.encode(pcs.protection_info()))
                    .unwrap_or_default(),
                self_added_pcs_base64: info
                    .self_added_pcs
                    .as_ref()
                    .map(|pcs| BASE64_STANDARD.encode(pcs.protection_info()))
                    .unwrap_or_default(),
                public_sharing_identity_base64,
                encrypted_public_sharing_key_base64: stable_url
                    .as_ref()
                    .and_then(|stable| stable.encrypted_public_sharing_key.as_ref())
                    .map(|value| BASE64_STANDARD.encode(value))
                    .unwrap_or_default(),
                protected_full_token_base64: stable_url
                    .as_ref()
                    .and_then(|stable| stable.protected_full_token.as_ref())
                    .map(|value| BASE64_STANDARD.encode(value))
                    .unwrap_or_default(),
                chain_private_key_base64: root_record
                    .chain_private_key
                    .as_ref()
                    .map(|value| BASE64_STANDARD.encode(value))
                    .unwrap_or_default(),
                chain_protection_base64: root_record
                    .chain_protection_info
                    .as_ref()
                    .map(|value| BASE64_STANDARD.encode(value.protection_info()))
                    .unwrap_or_default(),
                invited_pcs_base64: info
                    .invited_pcs
                    .as_ref()
                    .map(|value| BASE64_STANDARD.encode(value.protection_info()))
                    .unwrap_or_default(),
        ...
        if delete_after {
            let cleanup_result = container
                .perform(
                    &CloudKitSession::new(),
                    ZoneDeleteOperation::new(zone),
                )
                .await;
            if let Err(error) = cleanup_result {
                warn!("Failed to remove disposable iCloud Photos CMM probe zone: {}", error);
            }
        }
        create_result
    }

    pub async fn delete_icloud_photos_share_zone(
        &self,
        zone_name: String,
    ) -> Result<bool, WrappedError> {
        use rustpush::cloudkit::{CloudKitSession, ZoneDeleteOperation};

        let Some(uuid) = zone_name.strip_prefix("CMM-") else {
            return Err(WrappedError::GenericError {
                msg: "iCloud Photos share zone must start with CMM-".into(),
            });
        };
        uuid::Uuid::parse_str(uuid).map_err(|_| WrappedError::GenericError {
            msg: "iCloud Photos share zone has an invalid identifier".into(),
        })?;
        ...
        let (_, cloudkit) = create_keychain_clients(self).await?;
        let container = PHOTOS_CONTAINER.init(cloudkit).await?;
        let zone = container.private_zone(zone_name);
        container
            .perform(
                &CloudKitSession::new(),
                ZoneDeleteOperation::new(zone),
...
pub struct WrappedMultipartPart {
    pub part_index: u64,
...
pub struct WrappedMessage {
    ...
    pub participant_group_version: Option<u64>,
    ...
    // True when rustpush could decrypt an IDS payload but could not verify its
    // sender signature because the peer identity was unavailable or invalid.
    ...
    pub group_icon_version: Option<u64>,
...
fn message_inst_to_wrapped(msg: &MessageInst) -> WrappedMessage {
    ...
    let mut w = WrappedMessage {
        ...
        participant_group_version: None,
        attachments: vec![],
        reply_guid: None,
        reply_part: None,
        is_typing: false,
    ...
    match &msg.message {
        Message::Message(normal) => {
            ...
            append_wrapped_attachments(&mut w, &normal.parts, false);
        ...
        Message::React(react) => {
            ...
            match &react.reaction {
                ReactMessageType::React { reaction, enable } => {
                    ...
                    if let Reaction::Sticker { body, .. } = reaction {
                        append_wrapped_attachments(&mut w, body, true);
                    }
                }
        ...
        Message::ChangeParticipants(change) => {
            ...
            w.participant_group_version = Some(change.group_version);
        }
        Message::Typing(typing, app) => {
            // `false` is a stop-typing control, not the absence of a typing
            // event. Preserve event presence separately from its active state.
            w.is_typing = true;
            w.typing_active = Some(*typing);
            if let Some(app) = app {
        ...
        Message::IconChange(change) => {
            ...
            w.group_icon_version = Some(change.group_version);
        }
...
fn append_wrapped_attachments(
    wrapped: &mut WrappedMessage,
    parts: &MessageParts,
    force_sticker: bool,
) {
    for indexed_part in &parts.0 {
        let MessagePart::Attachment(att) = &indexed_part.part else {
            continue;
        };
        let (is_inline, inline_data, size, mmcs_descriptor_json) = match &att.a_type {
            AttachmentType::Inline(data) => (true, Some(data.clone()), data.len() as u64, None),
        ...
        wrapped.attachments.push(WrappedAttachment {
...
fn protect_private_state_file(path: &str) {
    #[cfg(unix)]
    {
        use std::os::unix::fs::PermissionsExt;
        if let Err(error) = std::fs::set_permissions(path, std::fs::Permissions::from_mode(0o600)) {
            if error.kind() != std::io::ErrorKind::NotFound {
                warn!("Failed to protect private state file {}: {}", path, error);
            }
        }
    }
    #[cfg(not(unix))]
...
pub fn init_logger() {
    ...
    // Migrate the old `<data-dir>/state/keystore.plist` layout if the new file
    // does not exist yet. Anchor even the legacy read beneath the selected
    // profile: an embedder's process cwd must never become credential input.
    let legacy_path = format!("{}/state/keystore.plist", xdg_dir);
    if !std::path::Path::new(&state_path).exists() {
        if std::path::Path::new(&legacy_path).exists() {
            match std::fs::copy(&legacy_path, &state_path) {
                Ok(_) => {
                    protect_private_state_file(&state_path);
                    info!("Migrated keystore from {} to {}", legacy_path, state_path);
                }
                Err(e) => warn!(
    ...
    // SoftwareKeystoreState contains APNs/IDS private keys. The profile parent
    // is already 0700, but enforce file-level least privilege as well and fix
    // files created by earlier iBlue builds.
    protect_private_state_file(&state_path);
    let path_for_closure = state_path.clone();
    init_keystore(SoftwareKeystore {
        state: RwLock::new(state),
        update_state: Box::new(move |s| {
            if plist::to_file_xml(&path_for_closure, s).is_ok() {
                protect_private_state_file(&path_for_closure);
            }
        }),
...
/// Resolve the data directory for process-local session state.
///
/// iBlue runs one process per Apple account. `IBLUE_DATA_DIR` lets each
/// process isolate its keystore, anisette data, and subsystem state from both
/// Corten and every other iBlue account. Corten's existing XDG behavior stays
/// as the fallback so this wrapper remains usable by the upstream bridge.
fn resolve_xdg_data_dir() -> String {
    if let Ok(data_dir) = std::env::var("IBLUE_DATA_DIR") {
...
/// a real Mac (e.g., via copper's QR code tool). Portable builds combine it
/// with open-absinthe's Unicorn NAC backend and a separately supplied,
/// allowlisted Apple IMDAppleServices binary.
///
...
fn _create_config_from_hardware_key_inner(base64_key: String, device_id: Option<String>) -> Result<Arc<WrappedOSConfig>, WrappedError> {
    ...
    // and then split. The portable NAC backend consumes the retained hardware
    ...
    // Some keys ship with empty `_enc` fields. The portable overlay leaves them
    // unchanged and supplies every field present in the extracted format to the
    // emulated Apple routine.
    ...
    // Build the MacOSConfig consumed by rustpush. Portable validation remains
    // fail-closed unless IBLUE_NAC_BINARY_FILE identifies the exact supported
    // external Apple binary.
...
/// A privacy-preserving trusted-phone choice for the interactive 2FA flow.
/// Apple exposes more phone metadata, but iBlue only needs the opaque ID for
/// the follow-up request and the final two digits so the user can choose.
#[derive(uniffi::Record, Clone)]
pub struct TwoFactorPhoneOption {
    pub id: u32,
...
pub async fn login_start(
    ...
    // not print the struct or its fields: native stderr is forwarded to the
    // TypeScript service log, which operators reasonably share for debugging.
    debug!("login_start: GSA client metadata prepared");
    // Persist the provisioned ADI machine in the absolute XDG data dir (same place
    // every other subsystem uses), NOT a cwd-relative path. With the relative
    // "state/anisette" the machine was re-provisioned on every login (state.plist
    // never found → exists=false → fresh machine + new X-Mme-Device-Id each run),
...
impl LoginSession {
    ...
    /// Identify how login_start expects the next code to arrive. A trusted-
    /// device challenge may still be switched to SMS with request_sms_2fa.
    pub fn two_factor_kind(&self) -> String {
        if self.sms_verify_body.lock().unwrap().is_some() {
            "sms".to_string()
        } else {
    ...
    pub async fn two_factor_phone_options(&self) -> Result<Vec<TwoFactorPhoneOption>, WrappedError> {
        ...
        Ok(extras
            ...
            .into_iter()
            .map(|phone| TwoFactorPhoneOption {
                id: phone.id,
    ...
    /// Switch an active trusted-device challenge to Apple's SMS flow. The
    /// returned VerifyBody is retained for submit_2fa, whose verification
    /// endpoint differs from the trusted-device endpoint.
    pub async fn request_sms_2fa(&self, phone_id: u32) -> Result<bool, WrappedError> {
        ...
        let result = account
            ...
            .await
            .map_err(|e| WrappedError::GenericError {
                msg: format!("Failed to send an SMS verification code: {}", e),
            })?;
        match result {
            icloud_auth::LoginState::NeedsSMS2FAVerification(body) => {
                *self.sms_verify_body.lock().unwrap() = Some(body);
                Ok(true)
            }
    ...
    pub async fn submit_2fa(&self, code: String) -> Result<bool, WrappedError> {
        ...
        // Keep the body until verification succeeds. Consuming it before the
        // request made a mistyped SMS code silently fall back to the wrong
        // trusted-device endpoint on the next attempt.
...
/// Remove this profile's current APNs device from Apple IDS. The caller must
/// retain the users, APNs state, and credentials until this returns success so
/// a transient network or Apple error remains retryable.
#[uniffi::export(async_runtime = "tokio")]
pub async fn deregister_account(
    config: &WrappedOSConfig,
    connection: &WrappedAPSConnection,
    users: &WrappedIDSUsers,
) -> Result<(), WrappedError> {
    let aps = connection.inner.state.read().await;
    deregister_ids(&*config.config, &aps, &users.inner)
        .await
        .map_err(|error| WrappedError::GenericError {
            msg: format!("Failed to deregister the IDS device: {}", error),
        })
}

/// Refresh the saved four-service IDS bundle exactly once without constructing
/// an `IMClient` or entering `ResourceManager`'s retry loop. This is a
/// Rust-only JSON-RPC helper, not part of the generated UniFFI surface.
pub async fn refresh_registration_once(
    config: &WrappedOSConfig,
    connection: &WrappedAPSConnection,
    users: &WrappedIDSUsers,
    identity: &WrappedIDSNGMIdentity,
) -> Result<Arc<WrappedIDSUsers>, WrappedError> {
    let mut refreshed_users = users.inner.clone();
    register(
    ...
    .await
    .map_err(|error| WrappedError::GenericError {
        msg: format!("registration refresh failed: {error}"),
    })?;
    Ok(Arc::new(WrappedIDSUsers {
        inner: refreshed_users,
    }))
}
...
async fn download_mmcs_attachments(
    ...
    let parts = match &msg_inst.message {
        Message::Message(normal) => Some(&normal.parts),
        Message::React(react) => match &react.reaction {
            ReactMessageType::React {
                reaction: Reaction::Sticker { body, .. },
                ..
            }
            | ReactMessageType::Extension { body, .. } => Some(body),
            _ => None,
        },
        _ => None,
    };
            ...
            if let AttachmentType::MMCS(mmcs) = &att.a_type {
                if att_idx < wrapped.attachments.len() {
                    match download_one_mmcs_attachment(mmcs, conn, &att.name).await {
                        ...
                        Err(e) => error!("Failed to download MMCS attachment {}: {}", att.name, e),
                    }
                }
            }
            att_idx += 1;
...
pub async fn new_client(
    ...
    // Keep the upstream `state/id_cache.plist` layout for compatibility, but
    // anchor it explicitly beneath IBLUE_DATA_DIR. Relying on process cwd made
    // the cache profile-local only because the TypeScript supervisor happened
...
impl Client {
    /// Resolve the exact iMessage recipients that `IMClient::send` will use
    /// before uploading an attachment to MMCS. A successful lookup is reused
    /// by the subsequent send through rustpush's one-minute cache floor, so
    /// this reorders the existing directory request instead of adding one.
    ///
    /// Without this guard, `Attachment::new_mmcs` transferred the complete file
    /// first and only then discovered `NoValidTargets`, leaving an orphaned
    /// Apple upload for an action that could never be delivered.
    async fn preflight_mmcs_upload_targets(
        &self,
        conversation: &WrappedConversation,
        handle: &str,
    ) -> Result<(), rustpush::PushError> {
        const TOPIC: &str = "com.apple.madrid";

        let handles = self.client.identity.get_handles().await;
        let targets = mmcs_preflight_targets(conversation, handle, &handles);
        self.client
            .identity
            .cache_keys(
                TOPIC,
        ...
        let delivery_targets = cache.get_participants_targets(TOPIC, handle, &targets);
        if targets.iter().any(|target| !handles.contains(target))
            && !delivery_targets
                .iter()
                .any(|target| !handles.contains(&target.participant))
        {
            return Err(rustpush::PushError::NoValidTargets);
        }
        Ok(())
    }
    ...
    /// Retry recoverable send failures while reusing the same MessageInst (and
    /// therefore the same UUID). For SendTimedOut this prevents duplicate UUIDs.
    /// For NoValidTargets, one forced cache refresh prevents rustpush's one-hour
    /// negative-cache lifetime from hiding a target that became available after
    /// Apple's registration/rate-limit state changed. The cache itself enforces
    /// its one-minute minimum refresh interval, so this cannot pound IDS.
    async fn send_with_flap_retry(
        ...
        let mut target_refresh_attempted = false;
        // The Matrix bridge historically used one immediate forced refresh to
        // recover from rustpush's negative cache. iBlue instead performs a
        // sparse, durable-cooldown preflight. Retrying here would amplify one
        // empty Apple directory response into up to three IDS queries.
        let allow_target_refresh = std::env::var_os("IBLUE_NO_VALID_TARGET_RETRY").is_none();
        loop {
            match self.client.send(msg).await {
                Ok(job) => return Ok(job),
                Err(rustpush::PushError::SendTimedOut) if timeout_retries < MAX_TIMEOUT_RETRIES => {
                    timeout_retries += 1;
                    warn!(
                ...
                Err(rustpush::PushError::NoValidTargets)
                    ...
                    let sender = msg.sender.as_deref().ok_or(rustpush::PushError::BadMsg)?;
                    let topic = if msg.message.is_sms() {
...
fn normalize_reply_target(
    ...
    let normalized_part = reply_guid.as_ref().map(|_| {
        reply_part
            .filter(|part| !part.is_empty())
            .unwrap_or_else(|| "0".to_string())
    });
    (reply_guid, normalized_part)
}

fn leave_group_participants(mut participants: Vec<String>, handle: &str) -> Vec<String> {
    participants.retain(|participant| participant != handle);
    participants
}

/// Build the same target set as `IMClient::send` without transmitting the
/// probe message. This deliberately delegates to `MessageInst::prepare_send`
/// so sender fanout and participant behavior cannot drift from rustpush.
fn mmcs_preflight_targets(
    conversation: &WrappedConversation,
    handle: &str,
    handles: &[String],
) -> Vec<String> {
    let mut probe = MessageInst::new(
        conversation.into(),
        handle,
        Message::Message(NormalMessage::new(String::new(), MessageType::IMessage)),
    );
    probe.prepare_send(handles)
}

fn inline_relay_attachment(
    data: Vec<u8>,
    mime: String,
    uti_type: String,
    filename: String,
...
mod reply_target_tests {
    ...
    #[test]
    fn missing_reply_part_defaults_to_first_part() {
        assert_eq!(
    ...
    #[test]
    fn stop_typing_remains_a_typing_event() {
    ...
    #[test]
    fn leave_group_omits_every_copy_of_the_local_handle() {
        assert_eq!(
            leave_group_participants(
                vec![
                    "mailto:self@example.com".to_string(),
                    "mailto:friend@example.com".to_string(),
                    "mailto:self@example.com".to_string(),
                ],
                "mailto:self@example.com",
            ),
            vec!["mailto:friend@example.com".to_string()],
        );
    }

    #[test]
    fn tapback_range_length_uses_nsstring_utf16_units() {
        assert_eq!(
            reaction_target("hello", Some(0)).associated_message_range_length(),
            5
        );
        assert_eq!(
            reaction_target("\u{fffc}", Some(0)).associated_message_range_length(),
            1
        );
        assert_eq!(
            reaction_target("😀", Some(0)).associated_message_range_length(),
            2
        );
        assert_eq!(
            reaction_target("ignored", None).associated_message_range_length(),
            u64::MAX
        );
    }

    #[test]
    fn mmcs_preflight_uses_the_send_recipient_and_sender_fanout_set() {
        let conversation = WrappedConversation {
            participants: vec!["mailto:friend@example.com".to_string()],
            group_name: None,
            sender_guid: None,
            is_sms: false,
    ...
    #[test]
    fn relay_attachment_keeps_bytes_inline_for_the_mms_serializer() {
        let attachment = inline_relay_attachment(
            vec![1, 2, 3],
            "image/png".to_string(),
        ...
        assert!(
            matches!(attachment.a_type, AttachmentType::Inline(ref data) if data == &[1, 2, 3])
        );
...
/// cache entry alone is insufficient for the canary: it may predate this
/// invocation. `response_entry_returned` proves this invocation replaced or
/// created the entry, including Apple's explicit empty-identity response.
#[derive(uniffi::Record, Clone)]
pub struct WrappedIdsLookupTarget {
    pub address: String,
    pub cache_entry_fresh: bool,
    pub response_entry_returned: bool,
    pub identity_count: u64,
    pub correlation_identifier_present: bool,
}

/// Diagnostic outcome for a strict, non-bisecting lookup invocation. iBlue's
/// native process also disables rustpush's internal error retry, so `attempts`
/// is one for every non-empty request and never expands per target.
#[derive(uniffi::Record, Clone)]
pub struct WrappedIdsLookupReport {
    pub targets: Vec<WrappedIdsLookupTarget>,
...
impl Client {
    ...
    ///
    /// Fails closed on every inconclusive shape — lookup error, caught panic,
    /// and an absent or stale cache entry. iBlue must not turn a failed iMessage
    /// into a successful-looking SMS unless IDS positively proves that another
    /// device on this account can relay it.
    pub async fn has_sms_relay(&self, handle: String) -> bool {
        ...
        if !report.poisoned.is_empty() {
            warn!("has_sms_relay: relay lookup panicked; refusing unproven SMS fallback");
            return false;
        }
        ...
        // anything else means the question went unanswered. An unanswered probe
        // is not permission to report a fallback send as successful.
        if !cache.does_not_need_refresh(SMS_TOPIC, &handle, &handle, false) {
            warn!("has_sms_relay: no fresh relay entry cached; refusing unproven SMS fallback");
            return false;
        }
    ...
    pub async fn validate_targets(
        ...
        // filter upstream applies (identity_manager.rs:866-870). Request refresh
        // so an empty answer does not remain authoritative for an hour; the cache
        // still enforces rustpush's one-minute minimum between forced refreshes.
        //
    ...
    /// Invoke the normal APNs-tunnel IDS lookup once, without ids_guard's
    /// panic bisection and without a fallback transport. The caller gets cache
    /// evidence even when the lookup errors, panics, or times out, but must only
    /// send when both required addresses were returned by this invocation with
    /// at least one live identity.
    pub async fn lookup_targets_once(
        &self,
        targets: Vec<String>,
        handle: String,
    ) -> WrappedIdsLookupReport {
        use futures::FutureExt;

        const TOPIC: &str = "com.apple.madrid";
        if targets.is_empty() {
            return WrappedIdsLookupReport {
                targets: Vec::new(),
                error: Some("strict IDS lookup requires at least one target".to_string()),
                panicked: false,
                timed_out: false,
                attempts: 0,
            };
        ...
        let before = {
            ...
            targets
                .iter()
                .map(|target| cache.get_cached_at_ms(TOPIC, &handle, target))
                .collect::<Vec<_>>()
        };
        ...
        .await;
        let (error, panicked, timed_out) = match outcome {
            Ok(Ok(Ok(()))) => (None, false, false),
            Ok(Ok(Err(error))) => (Some(error.to_string()), false, false),
            Ok(Err(_)) => (Some("IDS lookup panicked".to_string()), true, false),
            Err(_) => (
                Some("IDS lookup timed out after 30 seconds".to_string()),
                false,
                true,
            ),
        };
        ...
        let cache = self.client.identity.cache.lock().await;
        let inspected = targets
            .iter()
            .enumerate()
            .map(|(index, target)| {
                let cached_at = cache.get_cached_at_ms(TOPIC, &handle, target);
                WrappedIdsLookupTarget {
                    address: target.clone(),
                    cache_entry_fresh: cache.does_not_need_refresh(TOPIC, &handle, target, false),
                    response_entry_returned: cached_at.is_some() && cached_at != before[index],
                    identity_count: cache.get_keys(TOPIC, &handle, target).len() as u64,
                    correlation_identifier_present: cache
                        .get_correlation_id(TOPIC, &handle, target)
                        .is_some(),
                }
            })
            .collect();
    ...
    /// Run a read-only, directly signed HTTPS IDS lookup. Normal iMessage sends
    /// continue to use the APNs-tunnel cache; this exists to compare Apple's
    /// response across the two supported `id-query` transports.
    pub async fn validate_targets_http(
        &self,
        targets: Vec<String>,
        handle: String,
        ...
        self.client
            .identity
            .validate_targets_http(&targets, &handle)
            .await
    ...
    pub async fn send_message(
        ...
        if conversation.is_sms && !self.has_sms_relay(handle.clone()).await {
        ...
        // Parse iBlue's internal rich-link, iCloud-share, and poll prefixes. Their JSON is
        ...
        let (actual_text, link_meta, app) = if text.starts_with("\x00RL2\x01") {
            ...
            let url = string_value("url")
                .filter(|value| {
                    value.starts_with("https://share.icloud.com/photos/")
                        || value.starts_with("https://share.icloud.com.cn/photos/")
                        || value.starts_with("https://www.icloud.com/photos/#")
                        || value.starts_with("https://www.icloud.com.cn/photos/#")
                })
                .ok_or_else(|| WrappedError::GenericError {
            ...
            let spec = ExtensionApp {
                ...
                balloon: Some(Balloon {
                    ...
                    layout: Some(BalloonLayout::TemplateLayout {
                        image_subtitle: String::new(),
                        image_title: String::new(),
                        caption: "Sent a poll".to_string(),
                        secondary_subcaption: String::new(),
                        tertiary_subcaption: String::new(),
                        subcaption: String::new(),
                        class: rustpush::NSDictionaryClass::NSDictionary,
                    }),
                    ld_text: Some("Sent a poll".to_string()),
                    is_live: true,
                    icon: None,
                }),
            };
    ...
    /// parts. This mirrors BlueBubbles' private-API multipart operation while
    /// using the public rustpush message model instead of Messages.framework.
    pub async fn send_multipart_message(
        &self,
        conversation: WrappedConversation,
        parts: Vec<WrappedMultipartPart>,
        handle: String,
        subject: Option<String>,
        effect: Option<String>,
        reply_guid: Option<String>,
        ...
        if parts.is_empty() {
            return Err(WrappedError::GenericError {
                msg: "No multipart message parts provided".to_string(),
            });
        }

        // Validate every part before starting any MMCS upload so malformed
        // requests cannot leave a prefix of the attachments orphaned.
        for part in &parts {
            if part.data.is_some() {
                if part.filename.as_deref().unwrap_or("").is_empty()
                    || part.mime_type.as_deref().unwrap_or("").is_empty()
                    || part.uti_type.as_deref().unwrap_or("").is_empty()
                {
                    return Err(WrappedError::GenericError {
                        msg: format!(
                            "Multipart attachment part {} is missing filename, MIME type, or UTI",
                            part.part_index
                        ),
                    });
                }
            ...
            if part.mention.is_some() && part.text.as_deref().unwrap_or("").is_empty() {
                return Err(WrappedError::GenericError {
                    msg: format!("Multipart mention part {} has no text", part.part_index),
                });
            }
        }
        ...
        let has_attachments = parts.iter().any(|part| part.data.is_some());
        if has_attachments && !conversation.is_sms {
            self.preflight_mmcs_upload_targets(&conversation, &handle)
                .await
                .map_err(|e| WrappedError::GenericError {
                    msg: format!("Failed to resolve multipart attachment targets: {e}"),
                })?;
        }
        ...
        for part in parts {
            ...
            let message_part = if let Some(data) = part.data {
                let mime = part.mime_type.expect("validated multipart MIME type");
                let uti_type = part.uti_type.expect("validated multipart UTI type");
                let filename = part.filename.expect("validated multipart filename");
                let attachment = if conversation.is_sms {
                    ...
                    .await
                    .map_err(|e| WrappedError::GenericError {
                        msg: format!("Failed to upload multipart attachment: {e}"),
                    })?
                };
                MessagePart::Attachment(attachment)
            } else {
                let text = part.text.expect("validated multipart text");
                match part.mention.filter(|mention| !mention.is_empty()) {
                    Some(mention) => MessagePart::Mention(mention, text),
                    None => MessagePart::Text(text, Default::default()),
                }
            };
            message_parts.push(IndexedMessagePart {
                part: message_part,
                idx: Some(part_index),
                ext: None,
            });
        ...
        let mut msg = MessageInst::new(
            ...
            Message::Message(NormalMessage {
                parts: MessageParts(message_parts),
                effect,
                reply_guid,
                reply_part,
                service,
    ...
    pub async fn send_attachment(
        ...
        let is_voice = is_audio_message || uti_type == "com.apple.coreaudio-format";
        let service = if conversation.is_sms {
            MessageType::SMS {
                is_phone: false,
                using_number: handle.clone(),
        ...
        // Apple iMessage attachments use MMCS. Resolve the exact send target
        // set first so a missing recipient cannot orphan a completed upload.
        // SMS/MMS relay messages instead carry inline bytes; rustpush's relay
        // serializer rejects MMCS descriptors for that branch.
        let attachment = if conversation.is_sms {
            inline_relay_attachment(data, mime, uti_type, filename)
        } else {
            self.preflight_mmcs_upload_targets(&conversation, &handle)
                .await
                .map_err(|e| WrappedError::GenericError {
                    msg: format!("Failed to resolve attachment targets: {e}"),
                })?;
            let prepared = MMCSFile::prepare_put(Cursor::new(&data))
                .await
                .map_err(|e| WrappedError::GenericError {
                    msg: format!("Failed to prepare MMCS upload: {}", e),
                })?;
            Attachment::new_mmcs(
                &self.conn,
                &prepared,
            ...
            .await
            .map_err(|e| WrappedError::GenericError {
                msg: format!("Failed to upload attachment: {}", e),
            })?
        };
    ...
    pub async fn send_change_participants(
        &self,
        conversation: WrappedConversation,
        mut new_participants: Vec<String>,
        group_version: u64,
        handle: String,
    ) -> Result<String, WrappedError> {
        if conversation.is_sms {
    ...
    /// Leave an iMessage group chat. Apple represents this as the same command
    /// 190 participant-change payload used by add/remove, except the sender is
    /// deliberately absent from the complete target participant list.
    pub async fn send_leave_group(
        &self,
        conversation: WrappedConversation,
        group_version: u64,
        handle: String,
        ...
        if conversation.is_sms {
            return Err(WrappedError::GenericError {
                msg: "Leaving is only available for iMessage group chats".to_string(),
            });
        }
        let conv: ConversationData = (&conversation).into();
        let remaining = leave_group_participants(conv.participants.clone(), &handle);
        let mut msg = MessageInst::new(
            conv,
            &handle,
            Message::ChangeParticipants(ChangeParticipantMessage {
                new_participants: remaining,
                group_version,
            }),
        );
        self.send_with_flap_retry(&mut msg)
            .await
            .map_err(|e| WrappedError::GenericError {
                msg: format!("Failed to leave group: {}", e),
            })?;
        Ok(msg.id.clone())
    }

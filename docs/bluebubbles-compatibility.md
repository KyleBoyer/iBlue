...
| iCloud Photos image/video shares | Receive/resolver, existing-link send, and fresh creation live-verified | Photos-app balloons expose `iBlue.icloudShare` immediately. The additive resolver returns counts, dates, expiry, owner display name, ordered image/video items, dimensions, sizes, and authenticated proxy paths for original/medium/thumbnail variants without leaking Apple's temporary CloudKit token or CDN URL. `POST /api/v1/iblue/icloud-share` sends an existing link as the native Photos Messages app balloon rather than a plain link preview. The opt-in `icloud-web-setup` reuses the profile's encrypted saved password hash, asks for Apple's separate web MFA step, and stores the iCloud.com session in the same encrypted credential record. Fresh creation is live-verified end to end for JPEG, PNG, HEIC, H.264 MP4, and QuickTime MOV, including format-matched originals, bounded JPEG card previews, public-link resolution, native balloon delivery, and receipts. Apple's web importer returns status 415 for animated GIF, which remains supported through the ordinary iMessage attachment API. |

...
| Send/receive text | Live-verified in both directions | APNs text receive is verified without Messages.app. Text, subject, expressive effect, attributed body, and reply fields reach the raw IDS constructor. A live confetti receive verified Apple's `CKConfettiEffect` wire identifier; a friendly `flair: "confetti"` send then delivered to the peer and produced a correlated Apple delivery receipt. iBlue additionally exposes the normalized message-flair object while retaining BlueBubbles' raw field. |
| Send/receive attachments | Receive and audio send live-verified; remaining sends implemented | Live MMCS download, persistence, metadata, and the BB download route are verified, including received CAF audio with `isAudioMessage: true`. A fresh live audio message verified that Apple's `audio-transcription` attachment attribute survives as `iBlue.audioTranscription: {text, source: "apple"}` in the REST message object without speech recognition while the raw CAF remains downloadable; Socket.IO and webhooks reuse that same serialization. A generated M4A sent through `POST /message/attachment` with `isAudioMessage: true` rendered as an audio message on the peer and produced a correlated Apple delivery receipt. BB's staged `POST /attachment/upload` flow is also implemented; subject, effect, reply, and audio-message fields survive the API-to-IDS path. iMessage send proves the exact IDS recipient/fanout set before uploading MMCS bytes, while SMS relay attachments remain inline as rustpush requires. Live delivery of non-audio outbound attachments remains pending. |
| Apple rich links / Music shares | Receive live-verified | A live Apple Music app share arrived through `com.apple.messages.URLBalloonProvider` with its URL, title, and PNG artwork. Message objects expose normalized `iBlue.richLink` metadata, Apple Music storefront/catalog identifiers, and a downloadable artwork attachment while hiding the synthetic internal metadata attachment. |
| iCloud Photos image/video shares | Receive/resolver, existing-link send, and fresh creation live-verified | Photos-app balloons expose `iBlue.icloudShare` immediately. The additive resolver returns counts, dates, expiry, owner display name, ordered image/video items, dimensions, sizes, and authenticated proxy paths for original/medium/thumbnail variants without leaking Apple's temporary CloudKit token or CDN URL. `POST /api/v1/iblue/icloud-share` sends an existing link as the native Photos Messages app balloon rather than a plain link preview. The opt-in `icloud-web-setup` reuses the profile's encrypted saved password hash, asks for Apple's separate web MFA step, and stores the iCloud.com session in the same encrypted credential record; multipart `POST /api/v1/iblue/icloud-share/create` uploads one JPEG, creates and anonymously resolves a fresh public share, then sends it as the Photos balloon. A live fresh-link run reused Apple's deduplicated `cplAsset`, resolved the newly created share to its JPEG, delivered the native balloon to the peer, produced a correlated Apple delivery receipt, and was confirmed visible by the recipient. |
...
| Apple Messages Polls (“Choice”) | Send/receive live-verified | A five-option live poll was decoded with stable option identifiers. The peer's two GUI selections arrived as one full participant selection set, and a separate API-profile vote delivered as Apple's `amt=4000` Polls extension acknowledgement. `GET /api/v1/iblue/poll/:messageGuid` aggregates current votes by participant; `POST .../vote` replaces the sending profile's full multi-select set. |
...
| Typing and read state | Typing receive and delivery receipt live-verified; remaining paths implemented | A live typing-start event reached the webhook. Native conversion now preserves explicit stop-typing as `display:false`. A live outbound confetti message received and correlated Apple's delivery control. Peer delivery/read controls update the original outgoing message's `dateDelivered`/`dateRead` and emit `updated-message`; controls that race the send RPC are buffered until the outgoing `new-message` event is durable, and replayed controls are idempotent. Outbound typing, read, and BB's `POST /chat/:guid/unread` are implemented; the read-receipt round trip remains pending. |

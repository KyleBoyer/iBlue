...
| iCloud Photos image/video shares | Receive/resolver and outbound delivery live-verified | Photos-app balloons expose `iBlue.icloudShare` immediately. The additive resolver returns counts, dates, expiry, owner display name, ordered image/video items, dimensions, sizes, and authenticated proxy paths for original/medium/thumbnail variants without leaking Apple's temporary CloudKit token or CDN URL. `POST /api/v1/iblue/icloud-share` sends an existing link as the native Photos Messages app balloon rather than a plain link preview. Fresh link activation is unavailable because Apple accepts its private web-sharing fields only from an entitled system `cloudd` account context; iCloud.com's equivalent requires a separate cookie-backed web login that iBlue does not store. |

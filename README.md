...
npm run build:native     # builds the Rust engine (produces pkg/rustpushgo/target/release/iblue-native)
```
...
Start and keep the service running:

```bash
IBLUE_SERVER_PASSWORD='replace-me' npm run dev -- serve --profile secondary
```

For outbound testing against a known number, use the canary flow:
...
been verified; Windows is unbuilt and experimental. See
[platform support](docs/platform-support.md) and the
[BlueBubbles compatibility contract](docs/bluebubbles-compatibility.md). The
[same-user account isolation audit](docs/profile-isolation.md) documents why an
iBlue profile does not sign in, read from, or modify Messages.app. The
[exact API surface audit](docs/bluebubbles-api-surface.md) separates native
routes/events from explicit capability errors, and the
[official client bootstrap audit](docs/bluebubbles-client-bootstrap.md) traces
the stock app's connection, first-sync, and startup calls. The
[outbound wire audit](docs/outbound-wire-audit.md) separates offline protocol
evidence from operations that still require live delivery verification.

## iBlue prototype commands
...
The login prompt accepts `sms` when no trusted device is available. Profile
state lives under `~/Library/Application Support/iBlue/profiles/<name>` on
macOS. Apple credentials use the OS credential store; only APNs/IDS transport
state and the account identifier are written to `session.json`. See the
...
The repository is based on Corten-Matrix so its upstream documentation and
provenance remain below.
